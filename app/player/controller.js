...
module.exports = {
   ...
   checkout: async(req, res) => {
      try {
         ...
         if(!res_nominal) return res.status(404).json({ message: 'nominal voucher tidak ditemukan' })
         
         const res_payment = await Payment.findOne({ _id: payment })
         if(!res_payment) return res.status(404).json({ message: 'jenis pembayaran tidak ditemukan' })
